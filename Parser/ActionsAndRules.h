...
public:
  ...
  TypeTable<TypeTableRecord>& typetable()
  {
    return _typetable;
  }
...
public:
  ...
  void doAction(ITokCollection*& pTc)
  {
    ...
    {
      elem.type = "namespace";
      size_t pos = pTc->find("namespace");
      elem.name = (*pTc)[pos + 1];
      p_Repos->getNamespaces().push_back(elem.name);
    }
    else {
      elem.type = "unknown";
      elem.name = "anonymous";
...
public:
  ...
  void doAction(ITokCollection*& pTc)
  {
    ...
    {
      p_Repos->getNamespaces().pop_back();
...
// Action to add function name onto ScopeStack and to AST and TypeTable
...
public:
  ...
  void doAction(ITokCollection*& pTc)
  {
    ...
    if ((pTc->find("::") == pTc->length())&&(pTc->find("main") == pTc->length())) {
      TypeTableRecord record;
      record.name() = name;
      record.type() = "function";
      record.fileName() = p_Repos->getFileName();
      if (p_Repos->getNamespaces().size() < 1) {
        record.addNameSpace("");
      }
      else {
        //for (auto ns : p_Repos->getNamespaces()) {
        std::string ns = p_Repos->getNamespaces().front();
        record.addNameSpace(ns);
        //}
      }

      p_Repos->typetable().addRecord(record);
    }
  }
};
...
// Rule to detect aliases and typedef
class TypedefAndAlias : public IRule
{
public:
  bool isSpecialKeyWord(const std::string& tok)
  {
...
// Action to add declaration scope to AST and TypeTable
class AddTypedefAndAlias : public IAction
{
  Repository* p_Repos;
public:
  AddTypedefAndAlias(Repository* pRepos)
  {
    p_Repos = pRepos;
  }
